...
TIPOS disponíveis:
  ...
  python             - Python (guidelines completas)
  ...
  java               - Java (guidelines completas)
...
function show_menu() {
    ...
    echo -e "  3) Python (guidelines completas)"
    echo -e "  4) Python NLP/ML"
    echo -e "  5) FastAPI"
    echo -e "  6) TypeScript Core"
    echo -e "  7) React (TypeScript)"
    echo -e "  8) Docker"
    echo -e "  9) Kubernetes"
    echo -e " 10) Terraform"
    echo -e " 11) Ansible"
    echo -e " 12) Java Core"
    echo -e " 13) Java (guidelines completas)"
    echo -e " 14) Linux Hardening"
    echo -e " 15) Web (legacy)"
    echo -e " 16) DevOps (legacy)"
    echo -e " 17) SysAdmin (legacy)"
    echo -e " 18) Base (somente)"
    echo ""
    echo -e "  0) Sair"
    echo ""
}
...
function main() {
    ...
    if [[ $# -eq 0 ]]; then
        ...
        if [[ -z "$choices" ]]; then
            echo -e "${YELLOW}󰅙${NC} Nenhuma opção fornecida!"
            exit 1
        fi
        ...
        # Parse choices
        local type_map=("dotfiles" "python-core" "python" "python-nlp-ml" "fastapi" "typescript-core" "react" "docker" "kubernetes" "terraform" "ansible" "java-core" "java" "linux-hardening" "web" "devops" "sysadmin" "base")
        ...
        # Validar tipos
        local valid_types=("dotfiles" "python-core" "python" "python-nlp-ml" "fastapi" "typescript-core" "react" "docker" "kubernetes" "terraform" "ansible" "java-core" "java" "linux-hardening" "web" "devops" "sysadmin" "base")
        for type in "${types[@]}"; do
            local valid=false
