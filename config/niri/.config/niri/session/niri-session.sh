...
source "${XDG_CONFIG_HOME:-$HOME/.config}/niri/config.sh"
source "$SCRIPTS_LIB_DIR/logging.sh"

setup_logging "$DIR_LOG/niri-session.log" "Niri Session Init"
...
if [ -f "$WAYLAND_COMMON" ]; then
  ...
  source "$WAYLAND_COMMON" niri 2>&1
  set +a
...
log_success 'Ambiente configurado. Iniciando Niri...'
finish_logging 0 'Niri Session Init'
